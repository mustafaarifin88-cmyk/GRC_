...
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card shadow border-0 rounded-4 overflow-hidden">
            <div class="card-header border-bottom-0 py-4 text-center position-relative" style="background: linear-gradient(135deg, #4b6cb7 0%, #182848 100%);">
            ...
            <div class="card-body p-4 pt-5 mt-4">
                <form action="<?= base_url('kepalaunit/profil/update') ?>" method="POST" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    <div class="text-center mb-4">
                        <h4 class="fw-bold text-dark"><?= esc($user['nama_lengkap']) ?></h4>
                        <span class="badge rounded-pill px-3 text-white" style="background-color: #4b6cb7;"><?= esc($user['level']) ?></span>
                    </div>
                    ...
                    <div class="d-grid">
                        <button type="submit" class="btn text-white btn-lg rounded-pill shadow-sm" style="background: linear-gradient(135deg, #4b6cb7, #182848);"><i class="fas fa-save me-2"></i>Update Profil Saya</button>
                    </div>
                </form>
            </div>
        </div>
