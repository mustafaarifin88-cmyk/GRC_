...
<style>
    ...
    .nav-pills .nav-link.active { background: linear-gradient(135deg, #870000, #190A05); color: white; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
</style>
        ...
        <form action="" method="GET">
            <div class="row align-items-end">
                <div class="col-md-4 form-group mb-0">
                    ...
                    <select class="form-select border-danger shadow-sm" name="supervisor_id" required onchange="this.form.submit()">
                ...
                <div class="col-md-4 form-group mb-0">
                    ...
                    <select class="form-select border-danger shadow-sm" name="kepala_unit_id" <?= empty($list_ku) ? 'disabled' : '' ?>>
                ...
                <div class="col-md-4">
                    <button type="submit" class="btn btn-danger w-100 rounded-pill shadow-sm" style="background: linear-gradient(135deg, #870000, #190A05); border: none;"><i class="fas fa-filter me-2"></i>Terapkan Filter</button>
                </div>
            </div>
        </form>
    </div>
                ...
                <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    ...
                    foreach($labels as $key => $label): ?>
                        <button class="nav-link <?= $first ? 'active' : '' ?>" id="v-<?= $key ?>-tab" data-bs-toggle="pill" data-bs-target="#v-<?= $key ?>" type="button" role="tab">
                            <?= $label ?> <span class="badge bg-light text-danger float-end"><?= count($laporan[$key] ?? []) ?></span>
                        </button>
                    ...
                    foreach($labels as $key => $label): ?>
                        <div class="tab-pane fade <?= $first ? 'show active' : '' ?>" id="v-<?= $key ?>" role="tabpanel">
                            <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
                                <h5 class="fw-bold text-dark m-0">Menunggu Approval: <?= $label ?></h5>
                                <a href="<?= base_url('managerial/approval/export-excel/'.$key) ?>" class="btn btn-sm btn-success rounded-pill shadow-sm"><i class="fas fa-file-excel me-2"></i>Rekap Excel</a>
                            </div>
                                        ...
                                        <?php else: ?>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center" style="white-space: nowrap;">
                                                    <a href="<?= base_url('managerial/approval/export-pdf/'.$key.'/'.$row['id']) ?>" class="btn btn-sm btn-info text-white rounded-circle shadow-sm me-1" title="Cetak PDF"><i class="fas fa-file-pdf"></i></a>
                                                    <a href="<?= base_url('managerial/approval/approve/'.$key.'/'.$row['id']) ?>" class="btn btn-sm btn-success rounded-circle shadow-sm me-1" title="Approve" onclick="return confirm('Yakin ingin menyetujui laporan ini?')"><i class="fas fa-check"></i></a>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg">
            <div class="modal-header bg-danger text-white rounded-top-4">
                <h5 class="modal-title text-white"><i class="fas fa-exclamation-triangle me-2"></i>Tolak Laporan (Managerial)</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form action="<?= base_url('managerial/approval/reject') ?>" method="POST">
                <div class="modal-body p-4">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control border-danger border-opacity-25" name="alasan_tolak" rows="4" required placeholder="Jelaskan secara rinci alasan laporan ditolak..."></textarea>
                    </div>
                </div>
