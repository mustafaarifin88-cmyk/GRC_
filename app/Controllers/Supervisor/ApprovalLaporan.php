...
class ApprovalLaporan extends BaseController
{
    ...
    public function index()
    {
        ...
        $kuData = $this->hierarchyModel->select('tb_users.id, tb_users.nama_lengkap')
            ...
            if (!empty($staffIds)) {
                ...
                foreach ($types as $type) {
                    ...
                    $laporanData[$type] = $model->select($model->table . '.*, tb_users.nama_lengkap')
                                                ...
                                                ->where($model->table . '.status', 'approve_ku')
                                                ->findAll();
                }
            }
        }
        ...
        $data = [
            'title'      => 'Approval Laporan dari Kepala Unit',
            'list_ku'    => $kuData,
    ...
    public function detail($type, $id)
    {
        ...
        return view('supervisor/approval/detail_laporan', $data);
    }
        ...
        if(!empty($atasanIds)) {
            foreach($atasanIds as $atasan) {
                $this->notificationModel->insert([
                    ...
                    'judul'   => 'Laporan Staff Ditolak Supervisor',
                    'pesan'   => 'Laporan staff ditolak oleh Supervisor. Alasan: ' . $alasan
                ]);
            }
        }
