...
class ApprovalLaporan extends BaseController
{
    ...
    public function index()
    {
        ...
        $mgrData = $this->hierarchyModel->select('tb_users.id, tb_users.nama_lengkap')
                                        ->join('tb_users', 'tb_users.id = tb_user_hierarchy.bawahan_id')
                                        ->where('tb_user_hierarchy.atasan_id', $myId)
                                        ->findAll();

        $selectedMgr = $this->request->getGet('managerial_id');
        ...
        if ($selectedMgr) {
            $spvList = $this->hierarchyModel->select('tb_users.id, tb_users.nama_lengkap')
                                            ->join('tb_users', 'tb_users.id = tb_user_hierarchy.bawahan_id')
                                            ->where('tb_user_hierarchy.atasan_id', $selectedMgr)
                                            ->findAll();
                    ...
                    if (!empty($staffIds)) {
                        ...
                        foreach ($types as $type) {
                            ...
                            $laporanData[$type] = $model->select($model->table . '.*, tb_users.nama_lengkap')
                                                        ...
                                                        ->where($model->table . '.status', 'approve_mgr')
                                                        ->findAll();
                        }
                    }
                }
        ...
        $data = [
            'title'       => 'Approval Laporan dari Managerial',
            'list_mgr'    => $mgrData,
    ...
    public function detail($type, $id)
    {
        ...
        return view('pimpinan_tinggi/approval/detail_laporan', $data);
    }
        ...
        if(!empty($atasanIds)) {
            foreach($atasanIds as $atasan) {
                $this->notificationModel->insert([
                    ...
                    'judul'   => 'Laporan Staff Ditolak Pimpinan Tinggi',
                    'pesan'   => 'Laporan staff ditolak oleh Pimpinan Tinggi. Alasan: ' . $alasan
                ]);
            }
        }
